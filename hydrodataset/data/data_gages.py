...
from hydrodataset.utils.hydro_utils import is_any_elem_in_a_lst, unzip_nested_zip, hydro_logger, download_one_zip, \
    download_small_file
...
class Gages(DataSourceBase):
    ...
    def sites_in_one_region(self, region) -> dict:
        """
        Which sites in this region

        Parameters
        ----------
        region
            there are 10 regions in GAGES-II:
            ['allref', 'CntlPlains', 'EastHghlnds', 'MxWdShld', 'NorthEast', 'SECstPlain', 'SEPlains', 'WestMnts',
            'WestPlains', 'WestXeric']

        Returns
        -------
        dict
            sites in this region and their site-information
        """
        regions_shp_file_dir = self.data_source_description["GAGES_REGIONS_SHP_DIR"]
        regions_lst = ['allref', 'CntlPlains', 'EastHghlnds', 'MxWdShld', 'NorthEast', 'SECstPlain', 'SEPlains',
                       'WestMnts', 'WestPlains', 'WestXeric']
        if not (region in regions_lst):
            raise NotImplementedError("Please choose a region in the list:" + str(regions_lst))
        if region == "allref":
            region_name = "bas_ref_all"
        else:
            region_name = "bas_nonref_" + region
        region_shp_file = os.path.join(regions_shp_file_dir, region_name + ".shp")
        region_gdf = gpd.read_file(region_shp_file)
        ...
        out = dict()
        df_id_region = region_gdf["GAGE_ID"].values
        # assert (all(x < y for x, y in zip(df_id_region, df_id_region[1:])))
        for s in gage_fld_lst:
            if s is gage_fld_lst[1]:
                out[s] = data_all[data_all["STAID"].isin(df_id_region)][s].values.tolist()
            else:
                out[s] = data_all[data_all["STAID"].isin(df_id_region)][s].values
        return out
            ...
            if key == 'bas_classif':
                data_temp = pd.read_csv(data_file, sep=',',
                                        dtype={'STAID': str, "WR_REPORT_REMARKS": str, "ADR_CITATION": str,
                                               "SCREENING_COMMENTS": str}, engine='python', encoding='unicode_escape')
            elif key == 'bound_qa':
    ...
    def read_forcing_gage(self, usgs_id, var_lst, t_range_list, forcing_type='daymet'):
        ...
        data_folder = os.path.join(self.data_source_description["GAGES_FORCING_DIR"], forcing_type)
        ...
        data_file = os.path.join(data_folder, huc, '%s_lump_%s_forcing_leap.txt' % (usgs_id, forcing_type))
    ...
    def download_data_source(self):
        ...
        download_zip_files = [os.path.join(self.data_source_description["GAGES_DIR"], zip_file) for zip_file in
                              zip_files]
        ...
        unzip_dirs = [os.path.join(self.data_source_description["GAGES_DIR"], zip_file[:-4]) for zip_file in zip_files]
...
def prepare_usgs_data(data_source_description: Dict):
    ...
    # download zip files
    [download_one_zip(attr_url, data_source_description["GAGES_DIR"]) for attr_url in
     data_source_description["GAGES_DOWNLOAD_URL_LST"]]
    ...
    data_all = pd.read_csv(data_source_description["GAGES_GAUGE_FILE"], sep=',', dtype={0: str})
