...
class LAMAHICE(HydroDataset):
    """LamaHICE dataset class extending RainfallRunoff.

    This class provides access to the LamaHICE dataset, which contains hourly
    ...
    def __init__(self, data_path, region=None, download=False):
        ...
        """
        # Call parent class RainfallRunoff constructor with LamaHICE dataset
    ...
    def cache_attributes_xrdataset(self):
        ds_attr = self.aqua_fetch.fetch_static_features(self.aqua_fetch.stations(),self.aqua_fetch.static_features).to_xarray()
        BASE_UNITS = {
            # 地形特征
            "dis_m3_": "m^3/s",
            "run_mm_": "millimeter",
        ...
        ds_attr.to_netcdf(CACHE_DIR.joinpath("lamahice_attributes.nc"))
        return
    ...
    def cache_timeseries_xrdataset(self):
        ...
        # 保存文件
        batch_filename = f"lamahice_timeseries.nc"
    ...
    def read_attr_xrdataset(self, gage_id_lst=None, var_lst=None, **kwargs):
        ...
        try:
            attr = xr.open_dataset(CACHE_DIR.joinpath("lamahice_attributes.nc"))
        except FileNotFoundError:
            self.cache_attributes_xrdataset()
            attr = xr.open_dataset(CACHE_DIR.joinpath("lamahice_attributes.nc"))
        if var_lst is None or len(var_lst) == 0:
            var_lst = self.read_attr_all()
            return attr[var_lst].sel(id=gage_id_lst)
        else:
            return attr[var_lst].sel(id=gage_id_lst)
    ...
    def read_ts_xrdataset(
        ...
        **kwargs,
        ...
        if t_range is None:
            t_range = ["1950-01-01", "2021-12-31"]
        lamahice_tsnc = CACHE_DIR.joinpath("lamahice_timeseries.nc")
        if not os.path.isfile(lamahice_tsnc):
            self.cache_timeseries_xrdataset()
        ts = xr.open_dataset(lamahice_tsnc)
