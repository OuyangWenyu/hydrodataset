...
def main(args):
    ...
    data_dir = os.path.join(
        definitions.DATASET_DIR, "daymet4camels", "daymet_camels_671_bound"
    )
    # same name with CAMELS
    save_dir = os.path.join(
        definitions.DATASET_DIR, "daymet4camels", "basin_mean_forcing", "daymet"
    )
    ...
    for i in tqdm(range(len(basins_id))):
        ...
        save_file_i = os.path.join(
            huc02dir, basins_id[i] + "_lump_cida_forcing_leap_pet.txt"
        )
        frames_basin = []
        for j in tqdm(range(len(years)), leave=False):
            nc_path = os.path.join(
                data_dir,
                basins_id[i],
                basins_id[i] + "_" + str(years[j]) + "_boundary.nc",
            )
        ...
        df_i_intepolate = df_i.interpolate(
            method="linear", limit_direction="forward", axis=0
        )
        df_i_intepolate.to_csv(
            save_file_i, header=True, index=False, sep=" ", float_format="%.2f"
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Calculate basin mean forcings including PET."
    )
    parser.add_argument(
        "--year_range",
        dest="year_range",
