...
def main(args):
    ...
    camels = Daymet4Camels(
        os.path.join(definitions.DATASET_DIR, "daymet4camels"),
    ...
    save_dir = os.path.join(
        definitions.DATASET_DIR, "daymet4camels", "pbm_daily_dataset"
    )
    ...
    csv_date = pd.DataFrame(
        np.array(pd.to_datetime(t_range_list)).T, columns=pbm_index[0:1]
    )
    ...
    mean_forcings = camels.read_relevant_cols(
        basins_id, t_range, chosen_var, resample=0
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Get basin areas and basin mean values including P, PET, and Q"
    )
    parser.add_argument(
        "--year_range",
        dest="year_range",
