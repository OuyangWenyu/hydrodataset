...
def main(args):
    ...
    if dir_name == "daymet_camels_671_unmask":
        ...
        dir2 = os.path.join(
            definitions.DATASET_DIR, "daymet4basins", "daymet_camels_671_bound"
        )
    else:
    ...
    save_dir = os.path.join(
        definitions.DATASET_DIR, "daymet4camels", "daymet_camels_671_bound_resample"
    )
    ...
    for i in tqdm(range(len(basins_id))):
        ...
        for j in tqdm(range(len(years)), leave=False):
            ...
            nc_path = os.path.join(
                dir1, basins_id[i], basins_id[i] + "_" + str(years[j]) + file_name
            )
            if not os.path.isfile(nc_path):
                nc_path = os.path.join(
                    dir2, basins_id[i], basins_id[i] + "_" + str(years[j]) + file_name
                )
                if not os.path.isfile(nc_path):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dir",
        dest="dir",
        help="the directory of original data",
        default="daymet_camels_671_bound",
        type=str,
    )
