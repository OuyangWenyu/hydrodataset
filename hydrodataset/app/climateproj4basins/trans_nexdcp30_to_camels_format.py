...
def main(args):
    ...
    regions = ['allref', 'CntlPlains', 'EastHghlnds', 'MxWdShld', 'NorthEast', 'SECstPlain', 'SEPlains',
    ...
    for i in tqdm(range(len(years)), leave=False):
        for region in regions:
            ...
            trans_month_nex_dcp30to_camels_format(input_dir, output_dir, gage_dict, region, years[i])
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Trans NEX-DCP30 data of each basin to CAMELS format')
    parser.add_argument('--input_dir', dest='input_dir', help='The directory of downloaded NEX-DCP30 data',
                        default="D:\\data\\NEX-DCP30", type=str)
    parser.add_argument('--output_dir', dest='output_dir', help='The directory of transformed data',
                        default="D:\\data\\NEX-DCP30-CAMELS", type=str)
    parser.add_argument('--year_range', dest='year_range', help='The start and end years (right open interval)',
                        default=[2004, 2008], nargs='+')
