...
class NexGddpCmip6():
    def __init__(self):
        ...
        self.vars_type = ["hurs", "huss", "pr", "rlds", "rsds", "sfcWind", "tas", "tasmax", "tasmin"]
        self.url_pattern = "https://ds.nccs.nasa.gov/thredds2/ncss/AMES/NEX/GDDP-CMIP6/{gcm}/{scenario}/" \
    ...
    def download_all_nex_gddp_cmip6_for_a_region(self, north, east, south, west, save_dir):
        """
        ...
        for gcm in tqdm(self.gcms):
            for sce in tqdm(self.scenarios, leave=False):
                self.download_one_gcm_scenario_nex_gddp_cmip6_for_a_region(gcm, sce, north, east, south, west, save_dir)

    def download_one_gcm_scenario_nex_gddp_cmip6_for_a_region(self, gcm, sce, north, east, south, west, save_dir):
        """
        Download all NEX-GDDP-CMIP6 for a region
        ...
        for year in tqdm(np.arange(year_range[0], year_range[1] + 1)):
            for var in tqdm(self.vars_type, leave=False):
                self.download_one_nex_gddp_cmip6_file_for_a_region(gcm, sce, year, var, north, east, south, west,
                                                                   save_dir)

    def download_one_nex_gddp_cmip6_file_for_a_region(self, gcm, sce, year, var, north, east, south, west, save_dir):
        """
        Download all NEX-GDDP-CMIP6 for a region
        ...
        None
        """
        if gcm in ["CNRM-CM6-1", "CNRM-ESM2-1", "GISS-E2-1-G", "MIROC-ES2L", "UKESM1-0-LL"]:
            case = self.cases[1]
        ...
        if year < year_range[0] or year > year_range[1]:
            ...
            return
        if (gcm in ["BCC-CSM2-MR", "NESM3"] and var == "hurs") or (
                gcm in ["IPSL-CM6A-LR", "MIROC6", "NESM3"] and var == "huss") or (
                gcm in ["CESM2", "CESM2-WACCM", "IITM-ESM"] and var in ["tasmax", "tasmin"]):
            logging.warning("No available data for this" + gcm + " " + var)
            return
        url = self.url_pattern.format(gcm=gcm, scenario=sce, case=case, year=year, var=var, north=north,
                                      west=west, east=east, south=south)
        file_name = "{var}_day_{gcm}_{scenario}_{case}_gn_{year}.nc".format(gcm=gcm, scenario=sce, case=case, year=year,
                                                                            var=var)
