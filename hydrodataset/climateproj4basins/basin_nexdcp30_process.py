...
def trans_month_nex_dcp30to_camels_format(nexdcp30_dir, output_dir, gage_dict, region, year):
    """
    ...
    nexdcp30_dataset = ["gage_id", "time_start", "pr_mean", "pr_quartile25", "pr_median", "pr_quartile75",
                        "tasmin_mean", "tasmin_quartile25", "tasmin_median", "tasmin_quartile75", "tasmax_mean",
                        "tasmax_quartile25", "tasmax_median", "tasmax_quartile75"]
    camels_format_index = ["Year", "Mnth", "Day", "Hr", "pr_mean(kg/(m^2*s))", "pr_quartile25(kg/(m^2*s))",
                           "pr_median(kg/(m^2*s))", "pr_quartile75(kg/(m^2*s))", "tasmin_mean(K)",
                           "tasmin_quartile25(K)", "tasmin_median(K)", "tasmin_quartile75(K)", "tasmax_mean(K)",
                           "tasmax_quartile25(K)", "tasmax_median(K)", "tasmax_quartile75(K)"]
    ...
    for f_name in os.listdir(nexdcp30_dir):
        if fnmatch.fnmatch(f_name, 'nex_dcp30_' + region + '_mean_' + str(year) + '*.csv'):
            nexdcp30_data_file = os.path.join(nexdcp30_dir, f_name)

    data_temp = pd.read_csv(nexdcp30_data_file, sep=',', dtype={nexdcp30_data_file[0]: str})
    for i_basin in range(len(gage_dict[gage_id_key])):
        basin_data = data_temp[
            data_temp[nexdcp30_dataset[0]].values.astype(int) == int(gage_dict[gage_id_key][i_basin])]
        ...
        if basin_data.shape[0] > 12:
            ...
            if basin_data.shape[0] != 48:
                raise RuntimeError("Maybe you lose some RCP projections: 'rcp26', 'rcp45', 'rcp60', 'rcp85'")
        ...
        for i in range(len(new_data_df_lst)):
            ...
            output_file = os.path.join(
                output_huc_dir, gage_dict[gage_id_key][i_basin] + "_lump_nexdcp30_" + out_file_name_lst[i] + ".txt")
            print("output nexdcp30 " + out_file_name_lst[i] + " of", gage_dict[gage_id_key][i_basin], "year", str(year))
            if os.path.isfile(output_file):
                ...
                if year in years:
                    ...
                    _data_df = pd.concat([data_old, _data_df]).sort_values(by=camels_format_index[0:3])
