...
def trans_era5_land_to_camels_format(era5_land_dir, output_dir, gage_dict, region, year, time_zone="Asia/Hong_Kong"):
    """
    ...
    avg_dataset = ["gage_id", "time_start", "dewpoint_temperature_2m_mean", "temperature_2m_mean",
                   "skin_temperature_mean", "soil_temperature_level_1_mean", "soil_temperature_level_2_mean",
                   "soil_temperature_level_3_mean", "soil_temperature_level_4_mean", "lake_bottom_temperature_mean",
                   "lake_ice_depth_mean", "lake_ice_temperature_mean", "lake_mix_layer_depth_mean",
                   "lake_shape_factor_mean", "lake_total_layer_temperature_mean", "snow_albedo_mean", "snow_cover_mean",
                   "snow_density_mean", "snow_depth_mean", "snow_depth_water_equivalent_mean",
                   "temperature_of_snow_layer_mean", "skin_reservoir_content_mean",
                   "volumetric_soil_water_layer_1_mean", "volumetric_soil_water_layer_2_mean",
                   "volumetric_soil_water_layer_3_mean", "volumetric_soil_water_layer_4_mean", "forecast_albedo_mean",
                   "u_component_of_wind_10m_mean", "v_component_of_wind_10m_mean", "surface_pressure_mean",
                   "leaf_area_index_high_vegetation_mean", "leaf_area_index_low_vegetation_mean"]
    sum_dataset = ["gage_id", "time_start", "snowfall_hourly_sum", "snowmelt_hourly_sum",
                   "surface_latent_heat_flux_hourly_sum", "surface_net_solar_radiation_hourly_sum",
                   "surface_net_thermal_radiation_hourly_sum", "surface_sensible_heat_flux_hourly_sum",
                   "surface_solar_radiation_downwards_hourly_sum", "surface_thermal_radiation_downwards_hourly_sum",
                   ...
                   "evaporation_from_vegetation_transpiration_hourly_sum", "potential_evaporation_hourly_sum",
                   "runoff_hourly_sum", "snow_evaporation_hourly_sum", "sub_surface_runoff_hourly_sum",
                   "surface_runoff_hourly_sum", "total_evaporation_hourly_sum", "total_precipitation_hourly_sum"]
    camels_format_index = ['Year', 'Mnth', 'Day', 'Hr', 'dewpoint_temperature_2m', 'temperature_2m', 'skin_temperature',
                           'soil_temperature_level_1', 'soil_temperature_level_2', 'soil_temperature_level_3',
                           'soil_temperature_level_4', 'lake_bottom_temperature', 'lake_ice_depth',
                           'lake_ice_temperature', 'lake_mix_layer_depth', 'lake_shape_factor',
                           'lake_total_layer_temperature', 'snow_albedo', 'snow_cover', 'snow_density', 'snow_depth',
                           'snow_depth_water_equivalent', 'temperature_of_snow_layer', 'skin_reservoir_content',
                           'volumetric_soil_water_layer_1', 'volumetric_soil_water_layer_2',
                           'volumetric_soil_water_layer_3', 'volumetric_soil_water_layer_4', 'forecast_albedo',
                           'u_component_of_wind_10m', 'v_component_of_wind_10m', 'surface_pressure',
                           'leaf_area_index_high_vegetation', 'leaf_area_index_low_vegetation', 'snowfall_hourly',
                           'snowmelt_hourly', 'surface_latent_heat_flux_hourly', 'surface_net_solar_radiation_hourly',
                           'surface_net_thermal_radiation_hourly', 'surface_sensible_heat_flux_hourly',
                           ...
                           'evaporation_from_vegetation_transpiration_hourly', 'potential_evaporation_hourly',
                           'runoff_hourly', 'snow_evaporation_hourly', 'sub_surface_runoff_hourly',
                           'surface_runoff_hourly', 'total_evaporation_hourly', 'total_precipitation_hourly']
    ...
    for f_name in os.listdir(era5_land_dir):
        if fnmatch.fnmatch(f_name, 'era5_land_' + region + '_avg_mean_' + str(year) + '.csv'):
            avg_data_file = os.path.join(era5_land_dir, f_name)
        if fnmatch.fnmatch(f_name, 'era5_land_' + region + '_sum_mean_' + str(year) + '.csv'):
    ...
    for i_basin in range(len(gage_dict[gage_id_key])):
        ...
        avg_data_df = pd.DataFrame(avg_basin_data.iloc[:, 2:].values,
                                   columns=camels_format_index[4:4 + len(avg_dataset) - 2])
        sum_data_df = pd.DataFrame(sum_basin_data.iloc[:, 2:].values,
        ...
        output_file = os.path.join(output_dir, gage_dict[gage_id_key][i_basin] + '_lump_era5_land_forcing.txt')
