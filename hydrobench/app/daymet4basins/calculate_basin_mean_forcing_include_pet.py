"""
calculate basin mean forcings including calculated PET data
"""
...
from hydrobench.utils.hydro_utils import unserialize_geopandas, hydro_logger, t_range_days
from hydrobench.daymet4basins.basin_daymet_process import calculate_basin_mean
...
def main(args):
    ...
    camels_index = ['Year', 'Mnth', 'Day', 'Hr', 'dayl(s/day)', 'prcp(mm/day)', 'srad(W/m2)', 'swe(kg/m2)', 'tmax(C)',
                    'tmin(C)', 'vp(Pa)', 'petpt(mm/day)', 'petfao56(mm/day)']
    var = ["dayl", "prcp", "srad", "swe", "tmax", "tmin", "vp", "pet_pt", "pet_fao56"]

    data_dir = os.path.join(definitions.DATASET_DIR, "daymet4camels", "daymet_camels_671_bound")
    ...
    for i in tqdm(range(len(basins_id))):
        ...
        save_file_i = os.path.join(huc02dir, basins_id[i] + "_lump_cida_forcing_leap_pet.txt")
        frames_basin = []
        for j in tqdm(range(len(years)), leave=False):
            nc_path = os.path.join(data_dir, basins_id[i], basins_id[i] + "_" + str(years[j]) + "_boundary.nc")
...
# python calculate_basin_mean_forcing_include_pet.py --year_range 1990 2010
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Calculate basin mean forcings including PET.')
